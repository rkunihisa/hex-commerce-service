...
def make_orderline(
    sku: str = "SKU123", quantity: int = 1, unit_price: int = 1000, currency: str = "JPY"
) -> OrderLine:
    return OrderLine(sku=sku, quantity=quantity, unit_price=Money.from_major(unit_price, currency))


def make_order(
    id: str = "SKU123", currency: str = "JPY", lines: list[OrderLine] | None = None
) -> Order:
    if lines is None:
        lines = []
...
def test_orderline_invalid_quantity() -> None:
    with pytest.raises(NegativeQuantityError, match="order line quantity must be positive"):
...
def test_order_invalid_add_line() -> None:
    ...
    with pytest.raises(CurrencyMismatchError, match="line currency must match order currency"):
        order.add_line(line)
...
def test_order_invalid_add_item() -> None:
    order = make_order(lines=[])
    with pytest.raises(CurrencyMismatchError, match="unit price currency must match order currency"):
